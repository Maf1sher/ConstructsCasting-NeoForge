...
public class EnderbenderModifier extends Modifier implements SpellOnCastHook {
    ...
    public void afterSpellCast(IToolStackView tool, ModifierEntry modifier, LivingEntity caster, String spellId, SchoolType school, CastSource castSource) {
		if (validIds.contains(spellId) && !caster.hasEffect(MobEffectRegistry.EVASION)) {
            caster.addEffect(new MobEffectInstance(MobEffectRegistry.EVASION, 60, modifier.getLevel()-1, false, false, true)); //(modifier level) hits to be evaded over 3 seconds
        }
    }
}
